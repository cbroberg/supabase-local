...
- **OAuth redirect_uri** — Supabase local dev hardcodes the GoTrue callback to `http://127.0.0.1:54321/auth/v1/callback` by default. GitHub does an exact match and rejects it. Must explicitly set `redirect_uri = "https://supabase.db/auth/v1/callback"` under `[auth.external.github]` in `config.toml`, then `npm run restart`. Caddy header fixes (X-Forwarded-Proto) don't help — the URI is set before headers are ever read.
